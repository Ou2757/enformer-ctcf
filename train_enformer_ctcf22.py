...
# NKLS 开头的是未定位 scaffold；这里统一并入训练集，不放入验证集或测试集。
TRAIN_SCAFFOLD_PREFIXES = ("NKLS",)
...
def build_scaffold_mask(chrom):
    """筛选需要划入训练集的 scaffold 染色体 key。"""
    mask = np.zeros(chrom.shape, dtype=bool)
    for prefix in TRAIN_SCAFFOLD_PREFIXES:
        mask |= np.char.startswith(chrom, prefix)
    return mask
...
def build_manifest(data_dir, split_chroms, max_samples_per_split=None, seed=42):
    ...
    for path in npz_files:
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            for split, keys in split_key_sets.items():
                mask = np.isin(chrom, list(keys))
                if split == "train":
                    # 训练集额外接收 NKLS... scaffold；验证集/测试集只使用显式指定的 CM 染色体。
                    mask |= build_scaffold_mask(chrom)
